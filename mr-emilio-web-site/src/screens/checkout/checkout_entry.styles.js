...
export const CheckoutEntryTransition = styled.div`
  width: 100%;
  min-height: 100vh;
  ...
  @media (max-width: ${({ theme }) => theme.breakpoints.mobile}) {
    animation: ${({ $isExiting, $direction }) => {
      if (!$isExiting) {
        return css`
          checkout-entry-enter 260ms cubic-bezier(0.22, 1, 0.36, 1) both
        `;
      }

      if ($direction === "back") {
        return css`
          checkout-entry-exit-back 260ms cubic-bezier(0.22, 1, 0.36, 1)
            both
        `;
      }

      return css`
        checkout-entry-exit-forward 260ms cubic-bezier(0.22, 1, 0.36, 1)
          both
      `;
    }};

    @keyframes checkout-entry-enter {
      from {
        opacity: 0.96;
    ...
    @keyframes checkout-entry-exit-forward {
      from {
        opacity: 1;
        transform: translateX(0);
      }
    ...
    @keyframes checkout-entry-exit-back {
      from {
        opacity: 1;
        transform: translateX(0);
      }
