...
export const PickupStoreTransition = styled.div`
  width: 100%;
  min-height: 100vh;
  ...
  @media (max-width: ${({ theme }) => theme.breakpoints.mobile}) {
    animation: ${({ $isExiting, $direction }) => {
      ...
      if ($direction === "back") {
        return css`
          pickup-store-back 260ms
            cubic-bezier(0.22, 1, 0.36, 1) both
        `;
      }

      return css`
        pickup-store-forward 260ms
          cubic-bezier(0.22, 1, 0.36, 1) both
      `;
    }};

    @keyframes pickup-store-enter {
      from {
        opacity: 0.96;
    ...
    @keyframes pickup-store-forward {
      from {
        opacity: 1;
        transform: translateX(0);
      }
